...
var author$project$EchoBot$simply = F2(
	function (message, model) {
		return {
			command: elm$core$Platform$Cmd$none,
...
var author$project$EchoBot$update = F2(
	function (msg, model) {
		if (msg.$ === 'Fail') {
			var chat = msg.a;
			var reason = msg.b;
			return A2(
				author$project$EchoBot$simply,
				A2(author$project$Elmergram$answer, reason, chat),
				model);
		} else {
			var chat = msg.a;
			var joke = msg.b;
			return A2(
				author$project$EchoBot$simply,
				A2(author$project$Elmergram$answer, joke, chat),
				model);
		}
	});
...
var author$project$EchoBot$Fail = F2(
	function (a, b) {
		return {$: 'Fail', a: a, b: b};
	});
var author$project$EchoBot$NewJoke = F2(
	function (a, b) {
		return {$: 'NewJoke', a: a, b: b};
	});
var author$project$EchoBot$do = F3(
	function (message, model, cmd) {
		return {command: cmd, message: message, model: model};
	});
...
var author$project$EchoBot$handle = F2(
	function (newUpdate, model) {
		var _n0 = newUpdate.content;
		var message = _n0.a;
		var getJoke = elm$http$Http$request(
			{
				...
				expect: A2(
					...
					function (result) {
						if (result.$ === 'Ok') {
							var joke = result.a;
							return A2(author$project$EchoBot$NewJoke, message.chat, joke);
						} else {
							var reason = result.a;
							return A2(
								author$project$EchoBot$Fail,
								message.chat,
								elm$core$Debug$toString(reason));
						}
					},
		...
		return A3(
			author$project$EchoBot$do,
			elm$core$Maybe$Just(
				A2(author$project$Elmergram$answer, '', message.chat)),
			model,
			getJoke);
	});
...
var author$project$Main$EchoBotMsg = function (a) {
	return {$: 'EchoBotMsg', a: a};
};
...
var author$project$Main$cmdFromResponse = function (response) {
	return elm$core$Platform$Cmd$batch(
		_Utils_ap(
			_List_fromArray(
				[
					A2(elm$core$Platform$Cmd$map, author$project$Main$EchoBotMsg, response.command)
				]),
			A2(
				...
				A2(
					...
					A2(
						...
						author$project$Elmergram$encodeSendMessage,
						A2(elm$core$Basics$composeR, author$project$Main$sendMessage, elm$core$List$singleton)),
					response.message))));
};
var author$project$Main$updateFromResponse = function (response) {
	return _Utils_Tuple2(
		response.model,
		author$project$Main$cmdFromResponse(response));
};
var author$project$Main$handleUpdate = F2(
	function (newUpdate, model) {
		return author$project$Main$updateFromResponse(
			A2(author$project$EchoBot$handle, newUpdate, model));
	});
var author$project$Main$update = F2(
	function (msg, model) {
		if (msg.$ === 'NewUpdate') {
			...
			var botMsg = msg.a;
			return author$project$Main$updateFromResponse(
				A2(author$project$EchoBot$update, botMsg, model));
		}
	});
